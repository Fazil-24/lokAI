/**
 * Idempotent base-data seeding (MERGE, never CREATE) — shared by the CLI
 * seed script (scripts/seed.ts) and the admin-triggered POST /api/demo-reset
 * route, so both stay in sync with a single source of truth.
 */
...
import { ALL_SECTORS, deriveSector } from "./sectors";
...
async function seedPublicIndicators() {
  ...
  return PUBLIC_INDICATORS.length;
}
...
async function seedSchemeRules() {
  ...
  return SCHEME_RULES.length;
}

export interface SeedSummary {
  sanctionedWorks: number;
  locations: number;
  publicIndicators: number;
  schemeRules: number;
}

/** Re-seeds all base (real, non-citizen-generated) data. Safe to re-run any time. */
export async function seedBaseData(): Promise<SeedSummary> {
  const rows = readCsvRows();
  const blockNames = Array.from(
    new Set(rows.map((row) => canonicalizeBlockName(row.BLOCK || row.VILLAGE || row.CITY)))
  );

  await applyConstraints();
  await seedConstituency();
  await seedSectors();
  const locations = await seedLocations(blockNames);
  const sanctionedWorks = await seedSanctionedWorks(rows);
  const publicIndicators = await seedPublicIndicators();
  const schemeRules = await seedSchemeRules();

  return { sanctionedWorks, locations, publicIndicators, schemeRules };
}

/**
 * Deletes all citizen/officer-generated data (submissions, the issue themes
 * clustered from them, and any LLM-generated project options) while
 * preserving the real MPLADS/Census/scheme-rule base data. Used by
 * POST /api/demo-reset to return the graph to its clean pre-seeded state.
 */
export async function wipeDynamicData(): Promise<void> {
  await runWrite("MATCH (s:Submission) DETACH DELETE s");
  await runWrite("MATCH (p:ProjectOption) DETACH DELETE p");
  await runWrite("MATCH (t:IssueTheme) DETACH DELETE t");
}
